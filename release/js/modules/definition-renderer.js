...
`;}
const versionGroups=new Map();for(const[audience,app]of audienceMap.entries()){const version=app.version || 'N/A';if(!versionGroups.has(version)){versionGroups.set(version,{audiences:[],app: app
});}
versionGroups.get(version).audiences.push(audience);}
const sortedVersions=Array.from(versionGroups.entries()).sort(([a],[b])=>{if(a==='N/A' && b==='N/A')return 0;if(a==='N/A')return 1;if(b==='N/A')return-1;const parseVersion=(v)=>{const parts=v.split('.').map(part=> parseInt(part,10)|| 0);return parts;};const versionA=parseVersion(a);const versionB=parseVersion(b);for(let i=0;i < Math.max(versionA.length,versionB.length);i++){const partA=versionA[i]|| 0;const partB=versionB[i]|| 0;if(partA !==partB){return partB-partA;}
}
return 0;});let html=`
...
`;sortedVersions.forEach(([version,versionData],index)=>{const{audiences,app }=versionData;const collapseId=`definition-collapse-${index}`;const sortedAudiences=audiences.sort();const sourceTypes=new Set();for(const[audience,appData]of audienceMap.entries()){if(audiences.includes(audience)&& appData.sourceType){sourceTypes.add(appData.sourceType);}
}
let sourceBadgeHtml='';if(sourceTypes.size > 0){const sourceTypesArray=Array.from(sourceTypes);if(sourceTypesArray.length===1){sourceBadgeHtml=`<span class="source-badge">${sourceTypesArray[0]}</span>`;} else{sourceBadgeHtml=sourceTypesArray.map(type=>
`<span class="source-badge multiple">${type}</span>`).join('');}
}
html +=`
...
${sortedAudiences.map(audience=>{const shorthand=window.utils.getAudienceGroupShorthand(audience);return `<span class="audience-bubble" data-audience="${audience.toLowerCase()}">${shorthand}</span>`;}).join('')}
</div>
</h6>
${sourceBadgeHtml}
</div>
...
`;return html;}
renderAppProperties(app,appId,versionData=null,audienceMap=null){let versionSummaryHtml='';if(versionData && versionData.audiences.length > 1){const uniqueSourceTypes=[...new Set(versionData.audiences.map(audience=>{if(audienceMap && audienceMap.has(audience)){return audienceMap.get(audience).sourceType;}
return null;}).filter(Boolean))];versionSummaryHtml=`
...
if(value){const date=new Date(value);return `<span class="property-date">${date.toLocaleDateString()} ${date.toLocaleTimeString()}</span>`;}
return '<span class="property-empty">N/A</span>';case 'version':
...
return '<span class="property-empty">N/A</span>';case 'array':
if(Array.isArray(value)&& value.length > 0){return value.map(item=> `<span class="property-tag">${window.utils.escapeHtml(String(item))}</span>`).join('');}
...
return '<span class="property-empty">N/A</span>';case 'other':
if(typeof value==='object' && value !==null){if(Array.isArray(value)){if(value.length===0){return '<span class="property-empty">Empty array</span>';}
if(typeof value[0]==='object'){const uniqueId='arr-' + Math.random().toString(36).substr(2,9);return `<details class="property-object-details">
...
const uniqueId='obj-' + Math.random().toString(36).substr(2,9);return `<details class="property-object-details">
<summary class="property-object-summary">View Object(${Object.keys(value).length} properties)</summary>
<pre class="property-json">${JSON.stringify(value,null,2)}</pre>
</details>`;}
return window.utils.escapeHtml(String(value));default:
if(typeof value==='object' && value !==null){if(Array.isArray(value)){if(value.length===0){return '<span class="property-empty">Empty array</span>';}
return value.map(item=> `<span class="property-tag">${window.utils.escapeHtml(String(item))}</span>`).join('');}
const uniqueId='obj-' + Math.random().toString(36).substr(2,9);return `<details class="property-object-details">
<summary class="property-object-summary">View Object(${Object.keys(value).length} properties)</summary>
...
getNestedProperty(obj,path){return path.split('.').reduce((current,prop)=> current && current[prop],obj);}
formatPropertyName(key){return key.replace(/([A-Z])/g,' $1').replace(/^./,str=> str.toUpperCase());}
}
function toggleDefinitionCollapse(collapseId){const content=document.getElementById(collapseId);const indicator=document.getElementById(collapseId + '-indicator');if(content && indicator){const isCollapsed=content.classList.contains('collapsed');if(isCollapsed){content.classList.remove('collapsed');indicator.textContent='▼';} else{content.classList.add('collapsed');indicator.textContent='▶';}
}
}
window.toggleDefinitionCollapse=toggleDefinitionCollapse;window.DefinitionRenderer=DefinitionRenderer;window.definitionRenderer=new DefinitionRenderer();
