/**
 * Definition Renderer Module
 * Handles rendering app definition tab content and property formatting
 */
class DefinitionRenderer {
    ...
    renderAppProperties(app, appId, versionData = null, audienceMap = null) {
                    ...
                    <div class="category-properties">
                        ...
                        <div class="definition-property">
                            ...
                            <span class="property-value">${uniqueSourceTypes.map(type => 
                                `<span class="property-badge source-type">${type}</span>`
                            ).join('')}</span>
                        </div>
                        ` : ''}
                    </div>
        ...
        // Add version summary if applicable
        html += versionSummaryHtml;
    ...
    formatPropertyValue(value, type) {
        ...
        switch (type) {
            case 'url':
                return `<a href="${window.utils.escapeHtml(value)}" target="_blank" class="property-link">${window.utils.escapeHtml(value)}</a>`;
            
            case 'boolean':
                return `<span class="property-boolean ${value ? 'true' : 'false'}">${value ? 'Yes' : 'No'}</span>`;
            ...
            case 'date':
                if (value) {
                    ...
                    return `<span class="property-date">${date.toLocaleDateString()} ${date.toLocaleTimeString()}</span>`;
                }
                return '<span class="property-empty">N/A</span>';
...
// Make DefinitionRenderer available globally
window.DefinitionRenderer = DefinitionRenderer;
window.definitionRenderer = new DefinitionRenderer();
